# -*- coding: utf-8 -*-
"""
Created on Sat May 19 10:45:20 2018
...
class TrainingData(object):
    ...
    def generateData(self):
        ...
        data[:NumberOfDay]
        print(data)
        plt.title('Tesla Motors stock price dynamics')
        plt.xlabel('Period')
        plt.ylabel('Price')
        plt.plot(data)
        ...
        for x in range(len(data)-2): 
            ...
            file.write(str((float(cArray[x])-float(oArray[x+1]))/100) + ' ' + str((float(cArray[x+1]) - float(oArray[x+2]))/100) + ' ' +
